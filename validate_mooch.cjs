...
assert.equal(d.photo.length,41);assert.equal(d.forum.length,50);assert.equal(d.forum.filter(r=>r.status==='Текст статьи прочитан').length,47);assert.equal(d.combined.length,43);
