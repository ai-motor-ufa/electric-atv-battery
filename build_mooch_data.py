...
for row in csv.DictReader((ROOT/'mooch_photo.psv').open(),delimiter='|'):
    ...
    photo.append(row)
links=json.loads((ROOT/'forum_links.json').read_text())
...
forum=[]
for rec in links:
    r={'title':rec['title'],'url':rec['url'],'source_kind':'Форум','accessed':'2026-09-20'}
    r['rated_mAh']=int(re.search(r'(\d{4})mAh',rec['title'],re.I).group(1))
    r['model']=re.sub(r'^Bench (?:Re)?[Tt]est Results:\s*','',rec['title']).split(' - ')[0]
    r['status']='Только заголовок; текст не извлечён'
...
method=[
 ['Охват','41 строка фото с ёмкостью строго более 4000 мА·ч и 39 тем форума, найденных на первых четырёх страницах и в поиске. Полный архив не пройден: CAPTCHA.'],
 ['Дата','Фото обновлено 11.08.2026; сбор страниц 20.09.2026. Дата испытания с фото не равна дате публикации статьи.'],
 ...
 ['Доступность','Часть статей прочитана полностью, остальные содержат только заголовок и ссылку. Парсинг не подтверждает наличие товара в продаже.'],
...
payload={'date':'2026-09-20','photo_date':'2026-08-11','photo':photo,'forum':forum,'combined':combined,'method':method}
...
def write_csv(name,items):
    ...
    with (OUT/name).open('w',encoding='utf-8-sig',newline='') as f:
        w=csv.DictWriter(f,fieldnames=keys,delimiter=';');w.writeheader();w.writerows(items)
